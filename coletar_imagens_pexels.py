...
# --------- Chave Pexels ------------
# Primeiro tenta variável de ambiente, depois arquivo PEXELS_API_KEY.txt
PEXELS_API_KEY = os.environ.get("PEXELS_API_KEY")
if not PEXELS_API_KEY:
    key_file = Path(__file__).with_name("PEXELS_API_KEY.txt")
    if key_file.exists():
        PEXELS_API_KEY = key_file.read_text(encoding="utf-8").strip()

if not PEXELS_API_KEY:
    raise RuntimeError(
        "Chave do Pexels não encontrada. Crie um arquivo 'PEXELS_API_KEY.txt' ao lado deste .py "
        "contendo apenas a sua chave (ou defina a variável de ambiente PEXELS_API_KEY)."
    )
...
def coletar_para_classe(classe: str, termos: list[str], alvo: int):
    ...
    for u in urls_total:
        if u in seen:
            continue
        seen.add(u)
        urls_total_uniq.append(u)

    print(f"[INFO] Total de URLs únicas: {len(urls_total_uniq)}")
    ...
    with ThreadPoolExecutor(max_workers=MAX_THREADS) as ex:
        for i, url in enumerate(urls_total_uniq):
            nome = f"{i:05d}.jpg"
            dest = pasta_tmp / nome
